...
RUN set -e; \
    ...
    pip3 install 'numpy<2.4' 'torch<=2.9.1' torchvision torchaudio --index-url https://download.pytorch.org/whl/$USE_CUDA_DOCKER_VER --no-cache-dir; \
    else \
    pip3 install 'numpy<2.4' 'torch<=2.9.1' torchvision torchaudio --index-url https://download.pytorch.org/whl/cpu --no-cache-dir; \
    fi; \
    uv pip install --system -r requirements.txt --no-cache-dir

# Keep dependency installation cached when a model download needs to be retried.
# A verified bundle in deploy/offline-models is imported without model-network
# access. The empty tracked directory preserves the normal online build.
COPY ./deploy/model_cache.py /tmp/model_cache.py
RUN --mount=type=bind,source=deploy/offline-models,target=/offline-models \
    set -e; \
    python /tmp/model_cache.py warm --bundle /offline-models; \
