<script lang="ts">
	...
	import EnterpriseLogin from '$lib/components/auth/EnterpriseLogin.svelte';
	import { rememberSuccessfulCredentials } from '$lib/utils/remembered-credentials';
	...
	const signInHandler = async () => {
		...
		const sessionUser = await userSignIn(loginAccount, submittedPassword).catch((error) => {
			loginError = `${error}`;
			if (!$config?.features?.enable_ldap || $config?.onboarding) toast.error(loginError);
			return null;
		});
		if (!sessionUser) {
			loginError ||= $i18n.t('Unable to sign in. Check your connection and try again.');
			return;
		}

		if (enterpriseLogin)
			rememberSuccessfulCredentials('signin', rememberCredentials, loginAccount, submittedPassword);
	...
	const ldapSignInHandler = async () => {
		const loginAccount = ldapUsername.trim();
		const submittedPassword = password;
		const sessionUser = await ldapUserSignIn(loginAccount, submittedPassword).catch((error) => {
			loginError = `${error}`;
			if ($config?.onboarding) toast.error(loginError);
			return null;
		});
		if (!sessionUser) {
			loginError ||= $i18n.t('Unable to sign in. Check your connection and try again.');
		...
		if (enterpriseLogin)
			rememberSuccessfulCredentials('ldap', rememberCredentials, loginAccount, submittedPassword);
		await setSessionUser(sessionUser);
	};
...
{#if loaded && enterpriseLogin}
	<EnterpriseLogin
		bind:mode
		bind:ldapUsername
		bind:email
		bind:password
		bind:rememberCredentials
		bind:error={loginError}
		{submitting}
		allowEmail={$config?.features?.enable_login_form ?? false}
		hasCustomFooter={!!$config?.metadata?.login_footer}
		providers={$config?.oauth?.providers ?? {}}
		onSubmit={submitHandler}
	>
		{#if $config?.metadata?.login_footer}
			<div class="mt-3 marked">
				...
				<div class="w-full px-10 min-h-screen flex flex-col text-center">
					...
					{:else}
						<div class="my-auto flex flex-col justify-center items-center">
							<div
								id="auth-login-card"
								class=" sm:max-w-md my-auto pb-10 w-full dark:text-gray-100"
							>
